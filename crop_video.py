...
def compute_bbox_trajectories(trajectories, fps, frame_shape, video_path, image_shape=(256, 256), increase=0.1):
    commands = []
    for i, (bbox, tube_bbox, start, end) in enumerate(trajectories):
        if (end - start) > 30:
            command = compute_bbox(start, end, fps, tube_bbox, frame_shape, inp=video_path, image_shape=image_shape,
...
def process_video(video_path):
    ...
    try:
        for i, frame in tqdm(enumerate(video)):
            ...
            commands += compute_bbox_trajectories(not_valid_trajectories, fps, frame_shape, video_path)
    ...
    commands += compute_bbox_trajectories(trajectories, fps, frame_shape, video_path)
    return commands
